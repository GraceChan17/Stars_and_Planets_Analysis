...
files_in_data = sorted([file for file in os.listdir("data") if not file.startswith(".")], reverse=True, key=lambda x: x.split(os.path.sep))
files_in_data
# -
grader.check("q1")
...
file_paths = sorted([os.path.join("data", file) for file in files_in_data], reverse=True, key=lambda x: x.split(os.path.sep))
file_paths
# -
grader.check("q2")
...
csv_file_paths = sorted([path for path in file_paths if path.endswith(".csv")], reverse=True, key=lambda x: x.split(os.path.sep))
csv_file_paths
# -
grader.check("q3")
...
stars_paths = sorted([path for path in file_paths if os.path.basename(path).startswith("stars")], reverse=True, key=lambda x: x.split(os.path.sep))
stars_paths
# -
grader.check("q4")
...
row_idx = 2  # the index of the third star
...
total_lum = 0
num_stars = 0
for star in stars_1_dict.values():
    if star.stellar_luminosity is not None:
        total_lum += star.stellar_luminosity
        num_stars += 1

avg_lum_stars_1 = total_lum / num_stars
print(avg_lum_stars_1)
# -
grader.check("q7")
...
total_age = 0
num_stars = 0
for star in stars_2_dict.values():
    if star.stellar_age is not None:
        total_age += star.stellar_age
        num_stars += 1

avg_age_stars_2 = total_age / num_stars
print(avg_age_stars_2)
# -
grader.check("q8")
...
for star_file in stars_paths:
    stars_dict.update(get_stars(star_file))
# -
...
for star_name, star in stars_dict.items():
    if star.stellar_radius is not None and star.stellar_radius > max_radius:
        biggest_star = star_name
        max_radius = star.stellar_radius
...
num_stars = 0
for star_name, star in stars_dict.items():
    if star_name.startswith("Kepler") and star.stellar_age is not None:
        total_age += star.stellar_age
        num_stars += 1

avg_age_kepler = total_age / num_stars
avg_age_kepler
# -
...
def planet_cell(row_idx, col_name, planets_rows, header=planets_header):
    try:
        ...
        if col_name == "Controversial Flag":
            return bool(int(val))
...
row_idx = 4  # the index of the fifth planet
...
def get_planets(planet_file, mapping_file):   
    ...
    planets_data = process_csv(planet_file)
    planets_header = planets_data[0]
    planets_rows = planets_data[1:]
...
controversial_planets = [planet for planet in planets_2 if planet.controversial_flag]
...
planets_list = []
planet_files = sorted([file for file in os.listdir("data") if file.startswith("planets")])
mapping_files = sorted([file for file in os.listdir("data") if file.startswith("mapping")])

for planet_file, mapping_file in zip(planet_files, mapping_files):
    planets = get_planets(os.path.join("data", planet_file), os.path.join("data", mapping_file))
...
planets_disc_2023 = len([planet for planet in planets_list if planet.discovery_year == 2023])
planets_disc_2023
# -
grader.check("q17")
...
num_planets = 0
for planet in planets_list:
    if planet.planet_radius is not None:
        star = stars_dict[planet.host_name]
        if star.stellar_radius is not None and star.stellar_radius > 10:
            total_radius += planet.planet_radius
            num_planets += 1

avg_planet_radius_big_stars = total_radius / num_planets
...
youngest_star = None
for star_name, star in stars_dict.items():
    if star.stellar_age is not None and star.stellar_age < min_age:
        min_age = star.stellar_age
        youngest_star = star_name

youngest_star_planets = [planet for planet in planets_list if planet.host_name == youngest_star]
